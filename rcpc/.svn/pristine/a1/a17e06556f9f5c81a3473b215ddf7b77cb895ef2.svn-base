...
public class SendInvitationsAction extends ActionSupport {
	...
	public String getDataList() throws Exception {
		...
		ArrayList<PcEvaluateRelation> arr = pcSendInvitationsService.selectEname(pre);
	...
	public String doSendPeople() throws Exception {
		...
		String checkEname = req.getParameter("dataEname");
		
		//String checkEid = req.getParameter("checkEid");
		System.out.println("checkEname===========" + checkEname);
		...
		//分割多个checkEname
		String[] enames = checkEname.split(";");
		String repmMailcontent = "";
        String sendState = "";
		...
		for(int i=0;i<ids.length;i++){
			...
			sendUrl += "page/answers.jsp?aid=" + aid + "&hasRoot=true"+ "&eid=" + eids[i] + "&id=" + ids[i];
			
			repmMailcontent = mailcontent.replace("{被评估人姓名}", enames[i]);
			//Email doSen = new Email();
			isSuccess = email.sendEmail(sub, repmMailcontent, emails[i], "{链接地址}", sendUrl);
			if(isSuccess==true){
				sendState += "1;";
